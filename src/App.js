...
// Import the Posts (plural!) and SearchBar components, since they are used inside App componenti
...
const App = () => {
  ...
  const filteredPost = posts.filter(pst => {
    return pst.username.toLowerCase().includes( search.toLowerCase() )
  })
  console.log(filteredPost);
  console.log(posts)
  const likePost = postId => {
    setPost(posts.map(pst => {
      if (posts.id === postId){
